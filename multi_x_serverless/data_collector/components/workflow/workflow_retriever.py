...
class WorkflowRetriever(DataRetriever):
    ...
    def retrieve_workflow_summary(self, workflow_unique_id: str) -> dict[str, Any]:
        ...
        workflow_summarized_log: str = self._client.get_value_from_table(
        ...
        return self._consolidate_log(workflow_summarized_log)

    def _consolidate_log(self, log: str) -> dict[str, Any]:  # pylint: disable=too-many-branches, too-many-statements
        ...
        for instance_id, instance_data in loaded_data[  # pylint: disable=too-many-nested-blocks
            ...
            for region, region_data in instance_data["execution_summary"].items():
                ...
                if "init_data_transfer_size_samples" in region_data:
                    if "init_data_transfer_size_samples" not in consolidated[instance_id]["execution_summary"][region]:
                        consolidated[instance_id]["execution_summary"][region]["init_data_transfer_size_samples"] = []
                    consolidated[instance_id]["execution_summary"][region]["init_data_transfer_size_samples"].extend(
                        region_data["init_data_transfer_size_samples"]
                    )
