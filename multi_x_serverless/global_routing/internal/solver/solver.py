...
def filter_regions(regions: np.ndarray, workflow_description: dict) -> np.ndarray:
    if "filtered_regions" in workflow_description:
        return np.array(
            [region for region in regions if f"{region[0]}:{region[1]}" not in workflow_description["filtered_regions"]]
        )
...
def get_cost_matrix(regions: np.ndarray, functions: np.ndarray) -> np.ndarray:
    ...
    for i, function in enumerate(functions):
        for j, region in enumerate(regions):
            cost_matrix[i][j] = get_cost_for_region_function(region, function)
...
def get_runtime_array(regions: np.ndarray) -> np.ndarray:
    ...
    for i, region in enumerate(regions):
        runtime_array[i] = get_runtime_coefficient_for_region(region)
...
def get_latency_matrix(regions: np.ndarray) -> np.ndarray:
    ...
    for i, region1 in enumerate(regions):
        for j, region2 in enumerate(regions[i:], start=i):
            latency_matrix[i][j] = get_latency_coefficient_for_region(region1, region2)
...
def get_execution_carbon_matrix(regions: np.ndarray, functions: np.ndarray) -> np.ndarray:
    ...
    for i, function in enumerate(functions):
        for j, region in enumerate(regions):
            execution_carbon_matrix[i][j] = get_execution_carbon_for_region_function(region, function)
...
def get_egress_cost_matrix(regions: np.ndarray) -> np.ndarray:
    ...
    for i, region1 in enumerate(regions):
        for j, region2 in enumerate(regions[i:], start=i):
            egress_cost_matrix[i][j] = get_egress_cost_coefficient_for_region_and_destination_region(region1, region2)
