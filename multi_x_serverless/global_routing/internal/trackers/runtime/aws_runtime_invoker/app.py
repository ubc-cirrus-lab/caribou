    ...
    for function_name, FunctionArn in functions:
        ...
        for experiment_name, payload, execution_times in experiment_and_payload:
            ...
            results = (
                ...
                timing_result["min"],
                timing_result["max"],
                timing_result["5th_percentile"],
                timing_result["50th_percentile"],
                timing_result["90th_percentile"],
                timing_result["95th_percentile"],
                timing_result["99th_percentile"],
            )
...
def invoke_lambda_function(FunctionArn, function_region, payload=None):
    ...
    response = lambda_client.invoke(
        FunctionName=FunctionArn, InvocationType="RequestResponse", Payload=json.dumps(payload)
    )
...
def get_lambda_functions_with_name(name, region):
    ...
    filtered_functions = [
        ...
        if not name or name in func["FunctionName"]
    ]
