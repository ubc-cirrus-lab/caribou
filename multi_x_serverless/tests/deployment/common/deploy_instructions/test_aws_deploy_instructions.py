...
class TestAWSDeployInstructions(unittest.TestCase):
    @patch("builtins.open", new_callable=unittest.mock.mock_open, read_data='{"provider1": {"Version": "2012-10-17"}}')
    ...
    def test_get_deployment_instructions(self, mock_exists, mock_open):
        ...
        providers = {"provider1": {"config": {"memory": 128, "timeout": 3}}}
        runtime = "python3.8"
        handler = "handler"
        ...
        with patch(
            ...
            remote_state = RemoteState("provider1", "region1")
        function_exists = False
        provider1_deploy_instructions = AWSDeployInstructions("region1", Provider.AWS)

        instructions = provider1_deploy_instructions.get_deployment_instructions(
