...
from multi_x_serverless.routing.solver_inputs.components.data_sources.at.region_source import RegionSource
from multi_x_serverless.routing.solver_inputs.components.data_sources.from_to.instance_to_instance_source import (
    InstanceToInstanceSource,
)
from multi_x_serverless.routing.solver_inputs.components.data_sources.from_to.region_to_region_source import (
    RegionToRegionSource,
)
...
class DataSourceManager:
    ...
    def setup(
        ...
        self._instance_source.setup(loaded_data, instances, instance_indexer, configurations=instance_configuration)
    ...
    def get_region_to_region_data(self, data_name: str, from_region_index: int, to_region_index: int) -> Any:
    ...
    def get_region_data(self, data_name: str, region_index: int) -> Any:
        # Result type might not necessarily be float
        # Since region_source contains non-float data
        return self._region_source.get_value(data_name, region_index)

    def get_instance_data(self, data_name: str, instance_index: int) -> Any:
        # Result type might not necessarily be float
