...
class DistributionBFSFineGrainedSolver(DistributionSolver):
    ...
    def _distribution_solve(self, regions: list[str]) -> list[tuple[dict, float, float, float]]:
        ...
        processed_node_indices: set[
            int
        ] = set()  # Denotes the nodes that have been processed - used for clearing memory
        ...
        # Now go through the permitted regions and calculate the deployments
        ...
        deployments: dict[int, tuple[dict[int, int], Distribution, float, float]] = {}
        for current_region_index in permitted_regions:
            # The following variables are the current cost and carbon of the BFS

            ## Worse case cost and carbon
            cumulative_wc_cost = 0.0
            cumulative_wc_carbon = 0.0

            # Experimental Probabilitic Case, not sure if this can be done this way
            cumulative_pc_cost = 0.0
            cumulative_pc_carbon = 0.0

            # Cummulative Distribution of the cost and carbon
            cumulative_cost_distribution = np.empty((0,))
            cumulative_carbon_distribution = np.empty((0,))
