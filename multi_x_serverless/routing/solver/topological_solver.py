    ...
    def _solve(self, regions: np.ndarray) -> list[tuple[dict, float, float, float]]:
        ...
        for current_instance_index in topological_order:
            ...
            for previous_instance_index in prerequisites_indices:
                print('previous deployment length of instance:', previous_instance_index, '-', len(deployments[previous_instance_index]))
                previous_deployment = []
            ...
            if (len(previous_deployments) == 0): # If there is no previous deployment, add a dummy one  (Basically only for first node)
            ...
            if (current_instance_index == -1): # If this is the virtual end node
                for combination in itertools.product(*previous_deployments):
                    ...
                    for (original_deployment_placement, original_cost, original_carbon, original_runtime), previous_instance_index in combination:
