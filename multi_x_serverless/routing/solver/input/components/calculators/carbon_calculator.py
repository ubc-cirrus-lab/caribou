...
class CarbonCalculator(InputCalculator):
    def __init__(self, carbon_loader: CarbonLoader, datacenter_loader: DatacenterLoader, workflow_loader: WorkflowLoader, runtime_calculator: RuntimeCalculator) -> None:
        super().__init__()
    ...
    def calculate_transmission_carbon(
        ...
        if consider_probabilistic_invocations:
            ...
            return self._calculate_raw_transmission_carbon(from_instance_name, to_instance_name, from_region_name, to_region_name)
    
    def _calculate_raw_execution_carbon(self, instance_name: str, region_name: str, use_tail_latency: bool = False) -> float:
        ...
        average_cpu_power = self._datacenter_loader.get_average_cpu_power(region_name) # Get the average power consumption of the instance in the given region (kw_compute)
        average_memory_power = self._datacenter_loader.get_average_memory_power(region_name) # Get the average power consumption of the instance in the given region (kw_mb)
        cfe = self._datacenter_loader.get_cfe(region_name) # Get the carbon free energy of the grid in the given region
        pue = self._datacenter_loader.get_pue(region_name) # Get the power usage effectiveness of the datacenter in the given region
    ...
    def _calculate_raw_transmission_carbon(self, from_instance_name: str, to_instance_name: str, from_region_name: str, to_region_name: str):
        ...
        transmission_carbon_intensity = self._carbon_loader.get_transmission_carbon_intensity(from_region_name, to_region_name)
