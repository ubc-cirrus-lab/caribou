...
class WrapperOverheadDeploymentUtility():
    ...
    def deploy_experiment(self, directory_path: str) -> bool:
        config = self._file_utility.get_config(directory_path)
        if config != {}:
...
if __name__ == "__main__":
    deployment_utility = WrapperOverheadDeploymentUtility('us-west-2')
    ...
    files_list, directories_list = file_utility.get_files_and_directories(full_path)
