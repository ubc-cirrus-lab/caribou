...
class WorkflowInstance:
    ...
    def _handle_sns_invocation(self, edge_reached_time_to_sns_data: list[tuple[float, dict[str, Any]]]) -> float:
        ...
        if len(edge_reached_time_to_sns_data) > 0:
            ...
            if from_instance_node:
                ...
                self._manage_sns_invocation_data_transfer_dict(from_instance_node.sns_data_call_and_output_sizes, from_region_id, sns_data_transfer_size)
    ...
    def _manage_sns_invocation_data_transfer_dict(self, sns_data_transfer_dict: dict[str, list[float]], region_id: int, data_transfer_size: float) -> None:
