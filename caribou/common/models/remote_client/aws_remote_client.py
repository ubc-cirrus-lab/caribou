...
#     DELAY_TIME = 5
#     #CARIBOU_WORKFLOW_IMAGES_TABLE = "CaribouWorkflowImages"
...
#     #     image_name = f"workflow_{workflow_identifier}_image"
...
#     #         # If the repository exists, list images to check if we already have an image
#     #         response = ecr_client.list_images(repositoryName=repository_name)
#     #         if response['imageIds']:
#     #             # Assuming the latest image is what we want
#     #             image_uri = f"{self._get_ecr_registry()}/{repository_name}:latest"
#     #         else:
#     #             # If the repository is empty, proceed to create and push the image
#     #             image_uri = self._build_and_push_image(zip_contents, image_name, runtime, additional_docker_commands)
#     #     else:
#     #         # If the repository does not exist, create it and push the image
#     #         image_uri = self._build_and_push_image(zip_contents, image_name, runtime, additional_docker_commands)

#     #     # Deploy the Lambda function with the image_uri
#     #     role_arn = f"arn:aws:iam::{self._client('sts').get_caller_identity().get('Account')}:role/{role_identifier}"
#     #     kwargs: dict[str, Any] = {
...
#     #         "Handler": handler,
#     #         "Tags": {
#     #             "WorkflowIdentifier": workflow_identifier  # Adding workflow_identifier to Tags
#     #     }
#     #     }
        
#     #     # Conditional logic for timeout
#     #     if timeout >= 1:
...
#     #         # For update, we only need FunctionName and ImageUri
...
#         #         is_deployed = self._generate_dockerfile(runtime, handler, additional_docker_commands)
#         #         with open(os.path.join(tmpdirname, "Dockerfile"), "w", encoding="utf-8") as f_dockerfile:
#         #             f_dockerfile.write(is_deployed)
...
#     def _extract_workflow_name(self, function_name: str) -> str:
#         parts = function_name.split('-')
#         workflow_name = '-'.join(parts[:3])  # Join the first three parts to form the workflow name
...
#         additional_docker_commands: Optional[list[str]] = None,
#     ) -> str:
#         workflow_name = self._extract_workflow_name(function_name)
#         image_uri = self._get_or_build_image(
#             workflow_name, zip_contents, runtime, handler, additional_docker_commands
#         )
...
#     def _store_deployed_image_uri(self, workflow_name: str, image_uri: str) -> None:
#         self._workflow_image_cache[workflow_name] = {"image_uri": image_uri}

#         client = self._session.client("dynamodb", region_name=GLOBAL_SYSTEM_REGION)
#         client.update_item(
#             TableName=CARIBOU_WORKFLOW_IMAGES_TABLE,
#             Key={"key": {"S": workflow_name}},
#             UpdateExpression="SET #v = :value",
#             ExpressionAttributeNames={"#v": "value"},
#             ExpressionAttributeValues={":value": {"S": image_uri}},
#         )

#     def _get_deployed_image_uri(self, workflow_name: str) -> Optional[str]:
#         if workflow_name in self._workflow_image_cache:
#             return self._workflow_image_cache[workflow_name].get("image_uri")
...
#             image_uri = item["value"]["S"]
#             self._workflow_image_cache[workflow_name] = {"image_uri": image_uri}
#             return image_uri

#         return None

#     def _generate_dockerfile(self, runtime: str, lambda_handler: str, additional_docker_commands: Optional[list[str]] = None) -> str:
#         run_command = ""
...
#         # Generating the Dockerfile content
#         is_deployed = f"""
...
#         COPY app.py ./
#         COPY cli.py ./
#         COPY caribou_workflow.py ./
#         COPY workflow_builder.py ./
    
#         # Set environment variable for the Lambda handler
#         ENV AWS_LAMBDA_FUNCTION_HANDLER={lambda_handler}
    
#         # Specify the Lambda runtime's entry point
#         CMD ["app.lambda_handler"]
#         """
    
#         return is_deployed

#     def _build_docker_image(self, image_name: str, dockerfile_path: str, context_path: str) -> None:
#         command = ["docker", "build", "-t", image_name, "-f", dockerfile_path, context_path]
#         try:
...
#         repository_name = image_name.split(":")[0]
#         image_uri = f"{ecr_registry}/{image_name}"

#         try:
#             self._client("ecr").create_repository(repositoryName=repository_name)
#         except self._client("ecr").exceptions.RepositoryAlreadyExistsException:
#             pass  # Repository already exists, proceed

#         try:
#             subprocess.run(["docker", "tag", image_name, image_uri], check=True)
#             subprocess.run(["docker", "push", image_uri], check=True)
#             logger.info("Docker image %s pushed to ECR successfully.", image_uri)
#         except subprocess.CalledProcessError as e:
#             logger.error("Failed to push Docker image %s to ECR. Error: %s", image_uri, e)
#             raise
...
#         additional_docker_commands: Optional[list[str]] = None,
#     ) -> str:
#         deployed_image_uri = self._get_deployed_image_uri(workflow_name)
...
#             image_name = f"{workflow_name.lower()}:latest"
#             self._build_docker_image(tmpdirname, image_name)

#             # Upload the Image to ECR
#             image_uri = self._upload_image_to_ecr(image_name)
#             self._store_deployed_image_uri(workflow_name, image_uri)
...
#     def _create_lambda_function(self, kwargs: dict[str, Any]) -> (str, str): # type: ignore
#         client = self._client("lambda")
#         try:
#             response = client.create_function(**kwargs)
#         except ClientError as e:
#             logger.error("Error while creating Lambda function: %s", e)
#             raise

#         function_arn = response["FunctionArn"]
#         function_state = response.get("State")
#         return function_arn, function_state
...
#                     logger.info("Lambda function %s is active.", function_name)
#                     break
#                 time.sleep(self.DELAY_TIME)
#             except ClientError as e:
#                 logger.error("Error while checking function state: %s", e)
#                 break
...
class AWSRemoteClient(RemoteClient):  # pylint: disable=too-many-public-methods
    ...
    def get_predecessor_data(
        ...
        if item is not None and "message" in item:
            return item["message"]["SS"]
        return []
    def get_image(self,function_name, tmpdirname,runtime, handler, additional_docker_commands):
            logger.info(self.is_deployed)
            if self.is_deployed == False :
                ...
                self._store_deployed_image_uri("caribou", image_uri)
    ...
    def create_function(
        ...
        image_uri = self._get_deployed_image_uri(workflow_deployment_name)
        if len(image_uri) > 0:
            image_uri = self._copy_image_to_region(image_uri)
        ...
        if state != "Active":
            self._wait_for_function_to_become_active(function_name)
        print(f"Done creating function {function_name} with arn  {arn}")
        return arn
    ...
    def _store_deployed_image_uri(self, workflow_image_name: str, image_uri: str) -> None:
        print(f"workflow_image_name {workflow_image_name}")
        print(f"image_uri {image_uri}")

        workflow_instance_id = "-".join(workflow_image_name.split("-")[0:2])
        print(f"workflow_instance_id {workflow_instance_id}")

        # function_name_simple = function_name[len(workflow_instance_id) + 1 :].rsplit("_", 1)[0]
        # print(f"function_name_simple {function_name_simple}")

        # if workflow_instance_id not in self._workflow_image_cache:
        #     self._workflow_image_cache[workflow_instance_id] = {}

        self._workflow_image_cache[workflow_instance_id] = image_uri
    ...
    def _get_deployed_image_uri(self, workflow_deployment_name: str) -> str:

        workflow_instance_id = "-".join(workflow_deployment_name.split("-")[0:2])

        function_name_simple = workflow_deployment_name[len(workflow_instance_id) + 1 :].rsplit("_", 1)[0]
        ...
        item = response.get("Item")
        if item is not None and "value" in item:
            print(f"item {item} {item["value"]}  {item["value"]["S"]}")
            if workflow_instance_id not in self._workflow_image_cache:
                self._workflow_image_cache[workflow_instance_id] = {}
            self._workflow_image_cache[workflow_instance_id] = item["value"]["S"]
            return item["value"]["S"]
        return ""
    ...
    def create_workflow_ecr(self,workflow_deployment_name,zip_contents, runtime, handler, additional_docker_commands)-> str:
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            print(f"workflow_deployment_name {workflow_deployment_name}")
            # Step 3: Build the Docker Image
            workflow_image_name = f"{workflow_deployment_name.lower()}:latest"
            # image_name = "caribou:latest"

            self._build_docker_image(tmpdirname, workflow_image_name)

            # Step 4: Upload the Image to ECR
            image_uri = self._upload_image_to_ecr(workflow_image_name)
            self.image_uri=image_uri
            self._store_deployed_image_uri(workflow_image_name, image_uri)
            print(f"create_workflow_ecr {image_uri}")
        return image_uri
