...
#     # Deploy the single ECR image for the workflow
#     deployer.deploy_single_ecr_image(config.home_region)
...
# @cli.command("deploy_and_run", help="Deploy and run the workflow.")
# @click.argument("workflow_id", required=True)
# @click.option("--argument", "-a", help="The input to the workflow. Must be a valid JSON string.")
# @click.pass_context
# def deploy_and_run(ctx: click.Context, workflow_id: str, argument: Optional[str] = None) -> None:
#     ctx.invoke(deploy)
#     ctx.invoke(run, workflow_id=workflow_id, argument=argument)
