...
class WorkflowBuilder:
        ...
        while not functions_to_visit.empty():
            ...
            if function_instance_name not in function_instances:
                function_instances[function_instance_name] = FunctionInstance(
                    ...
                    entry_point=caribou_function.entry_point,
                    regions_and_providers=self._merge_and_verify_regions_and_providers(
                        caribou_function.regions_and_providers, config
                    ),
                    function_resource_name=caribou_function.name,
                )
                for successor_of_predecessor_i, successor in enumerate(
                    config.workflow_app.get_successors(caribou_function)
                ):
    ...
    def _get_function_name(self, config: Config, function: CaribouFunction, region: dict[str, str]) -> str:
    ...
    def _cycle_check(self, function: CaribouFunction, config: Config) -> None:
        visiting: set[CaribouFunction] = set()
        visited: set[CaribouFunction] = set()
        self._dfs(function, visiting, visited, config)

    def _dfs(
        self,
        node: CaribouFunction,
        visiting: set[CaribouFunction],
        visited: set[CaribouFunction],
        config: Config,
    ) -> None:
        visiting.add(node)
    ...
    def get_function_role(self, config: Config, function_name: str) -> IAMRole:
        ...
        if config.iam_policy_file:
            filename = os.path.join(config.project_dir, ".caribou", config.iam_policy_file)
        else:
            filename = os.path.join(config.project_dir, ".caribou", "iam_policy.yml")
