...
class TestDeploymentMigrator(unittest.TestCase):
    ...
    @patch("caribou.monitors.deployment_migrator.ReDeploymentUtility")
    def test_check(self, mock_re_deployment_utility):
        # Arrange
        ...
        mock_re_deployment_utility_instance = mock_re_deployment_utility.return_value
        mock_re_deployment_utility_instance.run.return_value = None
        ...
        mock_re_deployment_utility.assert_called_once_with("workflow1")
        mock_re_deployment_utility_instance.run.assert_called_once()
